...
class CustomRstConverter(RstConverter):
    ...
    def _get_table_row(self, test_case_result: Record_Object) -> list[str]:
        ...
        test_cases = test_case_result_attributes["relates"]
        test_cases_str = ""
        if test_cases is None:
            test_cases = self.rst_escape("N/A")
        else:
            if isinstance(test_cases, str):
                test_cases = [test_cases]

            for idx, test_case in enumerate(test_cases):
                anchor_tag = test_case.replace("SwTests.", "swe-test.rst-").lower()

                if 0 < idx:
                    test_cases_str += "\n"

                test_cases_str += "* " + self.rst_create_link(test_case, anchor_tag)
        ...
        row = [test_cases_str, test_function_name, test_result]
